...
| 7.65 | **Blog: Add 30 new categories** — Add comprehensive category list to `blog_categories` table: Lifestyle, Personal Development, Health & Fitness, Finance, Hospitality, Travel, Food & Recipes, Technology/Gadgets, Parenting, Family, Fashion & Style, DIY & Crafts, Development, Professional, Digital Marketing, Business & Leadership, Art & Design, Home & Interior Design, Sustainability/Eco-Living, Education & Coaching, Opinion, News, Reviews & Comparisons, Interviews & Profiles, Case Studies, Behind the Scenes, Spirituality, Love, Other, Creativity. Deduplicate against existing 32 categories in `INSERT_BLOG_CATEGORIES.sql`. | P0 | — | ☐ |
| 7.66 | **Blog: Submission-only model (no self-publishing)** — Users should NOT be able to publish blog posts directly. Replace the user blog editor (`UserBlogEditor.tsx`, routes `/blog/write` and `/blog/edit/:id`) with a Contributor Guidelines page. The page should display the full submission guidelines: pan-African thematic focus, tone/quality standards, submission process (email to editorial team), formatting requirements, and content policies. Posts can only be published by admin. | P0 | — | ☐ |
| 7.67 | **Streams Music: Freemium model (preview + purchase)** — Implement pay-per-song model where artists set their own price. Free users hear a 20-30 second preview of each song, then are prompted to purchase. Purchasing requires a registered, logged-in account. Needs: (1) `price` column on `songs` table (nullable — null = free), (2) `purchased_songs` junction table (user_id, song_id), (3) Audio player preview logic: play full song if free/purchased, else fade out at 20-30s and show purchase prompt, (4) Purchase flow UI: price display on song cards, purchase modal/button, payment integration (Bara Coins or external), (5) Artist upload form: add price field. | P0 | — | ☐ |
