...
export const TopBannerAd: React.FC<TopBannerAdProps> = ({ className = "" }) => {
  ...
  useEffect(() => {
    const fetchLatestActive = async () => {
      setLoading(true);
      try {
        // 1) Primary: only active banners with display_on_top = true
  ...
  return (
    <div 
      className={`w-full bg-white border-b border-gray-200 py-4 flex justify-center ${className}`}
                ...
                >
                  ...
                  >
                    <img
                      ...
                      className="w-full h-[90px] object-cover bg-gradient-to-r from-blue-50 to-indigo-50"
                    />
                  </div>
                </a>
              ) : (
